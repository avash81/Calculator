            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" inputMode="numeric" pattern="[0-9.]*" value={initial} onChange={e => setInitial(+e.target.value)} className="w-full h-12 px-4 rounded-xl border-2 border-gray-100 focus:border-blue-500 outline-none font-mono text-lg font-bold" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Final Value</label>
                  <input type="number" inputMode="numeric" pattern="[0-9.]*" value={final} onChange={e => setFinal(+e.target.value)} className="w-full h-12 px-4 rounded-xl border-2 border-gray-100 focus:border-blue-500 outline-none font-mono text-lg font-bold" />
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Duration (Years)</label>
                <input type="number" inputMode="numeric" pattern="[0-9.]*" value={years} onChange={e => setYears(+e.target.value)} className="w-full h-12 px-4 rounded-xl border-2 border-gray-100 focus:border-blue-500 outline-none font-mono text-lg font-bold" />
